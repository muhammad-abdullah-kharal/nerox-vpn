...
export class PaymentController {
  ...
  static async initializePayment(req: AuthRequest, res: Response) {
    try {
      ...
      await pool.query(
        `INSERT INTO payment_transactions (user_id, platform, product_id, purchase_token, status, amount, amount_paid)
         VALUES ($1, 'flutterwave', $2, $3, 'pending', $4, 0)`,
        [userId, plan.plan_id, txRef, plan.price_usd]
      );
    ...
    try {
      ...
      if (payload.event === 'charge.completed' && payload.data.status === 'successful') {
        ...
        const txRes = await pool.query(
          'SELECT * FROM payment_transactions WHERE purchase_token = $1 AND status = \'pending\'',
          [txRef]
        );
